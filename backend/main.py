...
# --- IMPORTS POUR FICHIERS STATIQUES (DOCKER) ---
...
origins = [
    "http://localhost:3000",       # React Dev (npm start)
    "http://127.0.0.1:3000",       # React Dev IP
    "http://localhost:8000",       # <--- INDISPENSABLE POUR DOCKER (Navigateur)
    "http://127.0.0.1:8000",       # <--- INDISPENSABLE POUR DOCKER (Navigateur)
...
try:
    ...
    print(f"⚠️ Erreur init Gemini (Clé probablement invalide): {e}")
...
def get_fallback_cities(country_name):
    ...
    return {"cities": ["Ville Exemple 1", "Ville Exemple 2"]}
...
def chat_gemini(request: schemas.ChatRequest, current_user: schemas.User = Depends(get_current_user)):
    ...
    """
    try:
        # On vérifie si model est bien initialisé (au cas où la clé serait bannie)
        ...
        # On ne plante pas l'app, on renvoie un message d'erreur propre
        return {"response": "Désolé, le service d'IA est momentanément indisponible (Clé API)."}
...
# =============================================================
# --- AJOUT CRITIQUE POUR LE DÉPLOIEMENT SINGLE CONTAINER ---
# =============================================================

# Configuration du dossier statique (React Build)
# En PROD (Docker), ce sera "./static".
# En DEV (Local), on essaie de trouver "../frontend/build".
static_folder = "static"
if not os.path.exists(static_folder) and os.path.exists("../frontend/build"):
    static_folder = "../frontend/build"

# On ne sert le frontend que si le dossier existe (évite de planter si on lance juste l'API)
if os.path.exists(static_folder):
    # 1. Servir les assets statiques (CSS, JS, Images du build React)
    app.mount("/static", StaticFiles(directory=f"{static_folder}/static"), name="static")
    
    # 2. Servir les fichiers à la racine du build (manifest.json, favicon.ico, logo...)
    @app.get("/{fname:path}")
    async def serve_static_root_file(fname: str):
        file_path = f"{static_folder}/{fname}"
        # Si le fichier existe physiquement, on le sert
        if os.path.isfile(file_path):
            return FileResponse(file_path)
        # Sinon, c'est une route React (ex: /login, /dashboard) -> on renvoie index.html
        return FileResponse(f"{static_folder}/index.html")
    
    # 3. Route racine explicite
    @app.get("/")
    async def serve_react_root():
        return FileResponse(f"{static_folder}/index.html")

else:
    print(f"⚠️ Mode API Seule : Dossier '{static_folder}' introuvable.")
